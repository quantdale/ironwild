...
function applyRatio(force) {
 ...
 try {
  ...
  /* lost context: keep the last good ratio */
 }
 if (composer) {
  try {
   composer.setPixelRatio(ratio);
  } catch (_) {
   /* composer swapped mid-flight: renderer is still scaled */
  }
 }
}
...
export function setContext(ctx) {
 ...
 renderer = r;
 composer =
  ctx.composer && typeof ctx.composer.setPixelRatio === "function"
   ? ctx.composer
   : null;
 ...
 * gets re-captured as basePR. A tier switch is the user re-baselining the
 * renderer on purpose, so the controller RESTARTS from the new tier's own
 * resolution: scale resets to 1.0 (never above it - see BOUNDS) and stale
 * frame statistics from the old tier are dropped. Carrying an adaptive scale
 * earned under the previous tier would silently undershoot the tier the user
 * just selected (e.g. picking Low during a slow spell and getting Low-5%).
 * No-op until setContext succeeded.
 */
