...
const CELL_KEYS = ['wood', 'shards', 'oil', 'medicine', 'arrows', 'fireArrows', 'hide'];
...
test('craft buttons disabled when resources are scarce', async ({ page }) => {
  ...
  await page.evaluate(() => {
    Object.assign(window.__IW.G.inventory, {
      wood: 0, shards: 0, oil: 0, medicine: 0,
      arrows: 0, fireArrows: 0, hide: 0,
    });
  });
...
test('enabled when affordable; real craft clicks consume and produce', async ({ page }) => {
  ...
  await page.evaluate(() => {
    Object.assign(window.__IW.G.inventory, {
      wood: 5, shards: 10, oil: 5, medicine: 0,
      arrows: 0, fireArrows: 0, hide: 4, armor: 0,
    });
  });
