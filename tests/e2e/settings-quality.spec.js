...
test('quality low<->high changes render resolution and persists', async ({ page }) => {
  await gotoGame(page);
  ...
  await expect.poll(renderState.bind(null, page)).toEqual({
    quality: 'low',
    pixelRatio: 1,
    canvasWidth: VIEW_W,
  });
  ...
  await expect.poll(() => renderState(page).then((s) => s.pixelRatio)).toBeCloseTo(1.5, 5);
...
test('quality choice survives a reload', async ({ page }) => {
  ...
  await expect.poll(() => renderState(page).then((s) => s.pixelRatio)).toBeCloseTo(1.25, 5);
  await page.locator('#iw-set-close').click();
