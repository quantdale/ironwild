...
test.setTimeout(SWGL_SPEC_MS); // software-GL/starved-host ceiling; hardware exits in seconds
...
test("interact collects a wood pickup exactly once", async ({ page }) => {
  ...
  // The E edge lands on the next simulated frame whenever it arrives; the
  // budget only has to outlast frame GAPS, never add delay on healthy hosts.
  await expect
    .poll(
      () =>
        page.evaluate(() => ({
